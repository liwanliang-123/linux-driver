...
struct work_struct work_key_test;
...
irq_handler_t test_key(int irq, void*args)
{
     ...
     // tasklet_schedule(&tasklet_key_test);
     schedule_work(&work_key_test);
...
int test_probe(struct platform_device *pedv)
{
     ...
     // tasklet_init(&tasklet_key_test, test, 100);
     INIT_WORK(&work_key_test, test);
